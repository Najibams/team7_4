...
# Bestanden inlezen
weather_file = r"/Users/moos/Downloads/weather_new(in).csv"
journey_file = r"/Users/moos/Downloads/147JourneyDataExtract30Jan2019-05Feb2019.csv"
stations_file = r"/Users/moos/Downloads/cycle_stations.csv"
...
start_date = st.sidebar.date_input("Selecteer startdatum", value=pd.to_datetime('2019-01-30'),
                                   min_value=pd.to_datetime('2019-01-30'),
                                   max_value=pd.to_datetime('2019-02-05'))
end_date = st.sidebar.date_input("Selecteer einddatum", value=pd.to_datetime('2019-02-05'),
                                 min_value=pd.to_datetime('2019-01-30'),
                                 max_value=pd.to_datetime('2019-02-05'))
...
temp_min, temp_max = st.sidebar.slider(
    "Temperatuur",
    min_value=int(weather_df['avg_temp'].min()),
    max_value=int(weather_df['avg_temp'].max()),
    value=(0, 10)
)
...
journey_df_filtered = journey_df[(journey_df['Start Date'].dt.date >= start_date) &
                                 (journey_df['Start Date'].dt.date <= end_date)]
...
journey_with_stations = journey_with_start.merge(
    stations_df, left_on='EndStation Name', right_on='name', how='left', suffixes=('', '_end')
)
...
if set(required_cols).issubset(merged_data.columns):
    merged_data['distance'] = merged_data.apply(
        lambda row: great_circle(
            (row['lat_start'], row['long_start']),
            (row['lat_end'], row['long_end'])
        ).meters, axis=1
    )
else:
...
if page == "📈 Analyse":
    ...
    if show_boxplots:
        ...
        if plot_type == "Gemiddelde Temperatuur":
            ...
            if forecast_checkbox_temp:
                ...
                predicted_temps = [5, 4.5, 4, 4]  # Temperaturen voor 6, 7, 8 en 9 februari
            ...
            if forecast_checkbox_rides:
                ...
                predicted_rides = [25500, 23000, 22500, 21000, 21000]  # Voorspellingen voor 5, 6, 7, 8 en 9 februari
    ...
    with col1:
        st.subheader("📍 Fietsreizen op de Kaart")
    ...
    st.write("Dit dashboard geeft een overzicht van alle gemaakte fietstochten, inclusief een interactieve kaart en statistieken over de afstanden. 🚴‍♂️🔥")
